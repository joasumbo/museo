

<?php $__env->startSection('title', 'Horários - Museu Municipal de Alcanena'); ?>
<?php $__env->startSection('description', 'Horários de funcionamento do Museu Municipal de Alcanena'); ?>

<?php $__env->startSection('content'); ?>
<!--==============================
Breadcumb
============================== -->
<div class="breadcumb-wrapper" data-bg-src="<?php echo e(asset('assets/img/gallery/DSC_3893.jpg')); ?>" style="position: relative;">
    <div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5);"></div>
    <div class="container" style="position: relative; z-index: 1;">
        <div class="breadcumb-content" style="padding: 60px 0;">
            <h1 class="breadcumb-title">Horários de Funcionamento</h1>
            <ul class="breadcumb-menu">
                <li><a href="<?php echo e(route('home')); ?>">Início</a></li>
                <li>Horários</li>
            </ul>
        </div>
    </div>
</div>

<!--==============================
Opening Hours Section
============================== -->
<section class="space" style="padding: 80px 0;">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10 col-xl-8">
                <div class="title-area text-center mb-50">
                    <span class="sub-title" style="color: #D4A574; font-size: 16px; font-weight: 600; text-transform: uppercase; letter-spacing: 2px;">
                        Planeie a Sua Visita
                    </span>
                    <h2 class="sec-title" style="font-size: 42px; font-weight: 700; margin-top: 15px; color: #1a1a1a;">Horários de Funcionamento</h2>
                    <p style="font-size: 18px; color: #666; margin-top: 20px; line-height: 1.7;">
                        O Museu Municipal de Alcanena está aberto ao público durante todo o ano.<br>
                        <strong>Período Atual:</strong> <?php echo e($currentSeason == 'summer' ? 'Verão' : 'Inverno'); ?>

                    </p>
                </div>

                <div class="schedule-wrapper" style="background: #fff; border-radius: 15px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); overflow: hidden;">
                    <?php
                        $diasSemana = [
                            'monday' => 'Segunda',
                            'tuesday' => 'Terça',
                            'wednesday' => 'Quarta',
                            'thursday' => 'Quinta',
                            'friday' => 'Sexta',
                            'saturday' => 'Sábado',
                            'sunday' => 'Domingo'
                        ];
                        
                        // Reordenar para começar na segunda
                        $allDays = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'];
                        $schedulesArray = $schedules->keyBy('day_of_week');
                    ?>

                    <?php $__currentLoopData = $allDays; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                            $schedule = $schedulesArray->get($day);
                        ?>
                        <div class="schedule-item" style="border-bottom: 1px solid #f0f0f0; padding: 25px 40px; display: flex; align-items: center; justify-content: space-between; transition: all 0.3s;">
                            <div class="schedule-day" style="flex: 0 0 150px;">
                                <h4 style="font-size: 18px; font-weight: 600; color: #1a1a1a; margin: 0;"><?php echo e($diasSemana[$day]); ?></h4>
                            </div>
                            <div class="schedule-time" style="flex: 1; text-align: center;">
                                <?php if($schedule && $schedule->is_open): ?>
                                    <div style="display: inline-flex; align-items: center; gap: 30px;">
                                        <?php if($schedule->morning_open && $schedule->morning_close): ?>
                                            <span style="color: #D4A574; font-weight: 600; font-size: 16px;">
                                                <i class="far fa-sun" style="margin-right: 8px;"></i>
                                                <?php echo e(substr($schedule->morning_open, 0, 5)); ?> - <?php echo e(substr($schedule->morning_close, 0, 5)); ?>

                                            </span>
                                        <?php endif; ?>
                                        <?php if($schedule->afternoon_open && $schedule->afternoon_close): ?>
                                            <span style="color: #D4A574; font-weight: 600; font-size: 16px;">
                                                <i class="far fa-moon" style="margin-right: 8px;"></i>
                                                <?php echo e(substr($schedule->afternoon_open, 0, 5)); ?> - <?php echo e(substr($schedule->afternoon_close, 0, 5)); ?>

                                            </span>
                                        <?php endif; ?>
                                    </div>
                                <?php else: ?>
                                    <span style="color: #999; font-weight: 500; font-size: 15px;">
                                        <i class="fas fa-ban" style="margin-right: 8px;"></i>
                                        Encerrado
                                    </span>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>

                <div class="text-center mt-50">
                    <div class="alert" style="background: #f8f9fa; border-left: 4px solid #D4A574; padding: 20px 30px; text-align: left; display: inline-block; max-width: 600px; border-radius: 8px;">
                        <i class="fas fa-info-circle" style="color: #D4A574; font-size: 20px; margin-right: 10px;"></i>
                        <span style="color: #555; font-size: 15px; line-height: 1.6;">
                            <strong>Nota:</strong> O museu permanece encerrado nos feriados nacionais e municipais. Sugerimos que confirme a disponibilidade antes da sua visita.
                        </span>
                    </div>
                </div>

                <div class="text-center mt-40">
                    <a href="<?php echo e(route('agendar.visita')); ?>" class="btn" style="padding: 15px 40px; font-size: 16px; font-weight: 600;">
                        <i class="fas fa-calendar-check me-2"></i>Agendar Visita
                    </a>
                    <a href="<?php echo e(route('contactos')); ?>" class="btn btn-border ms-3" style="padding: 15px 40px; font-size: 16px; font-weight: 600;">
                        <i class="fas fa-phone me-2"></i>Contacte-nos
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laravel_projects\museo\resources\views/horarios.blade.php ENDPATH**/ ?>