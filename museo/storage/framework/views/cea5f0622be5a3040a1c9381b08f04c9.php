...
<?php $__env->startSection('page-title', 'Editar Evento'); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="<?php echo e(route('admin.events.index')); ?>" class="inline-flex items-center text-sm text-gray-400 hover:text-white">
    ...
    <div class="bg-admin-card border border-admin-border rounded-xl">
        <div class="px-6 py-4 border-b border-admin-border flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500"><?php echo e($event->title); ?></p>
            </div>
        ...
        <form action="<?php echo e(route('admin.events.update', $event)); ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="exhibition" <?php echo e(old('type', $event->type) === 'exhibition' ? 'selected' : ''); ?>>Exposição</option>
                        <option value="workshop" <?php echo e(old('type', $event->type) === 'workshop' ? 'selected' : ''); ?>>Workshop</option>
                        <option value="conference" <?php echo e(old('type', $event->type) === 'conference' ? 'selected' : ''); ?>>Conferência</option>
                        <option value="guided_tour" <?php echo e(old('type', $event->type) === 'guided_tour' ? 'selected' : ''); ?>>Visita Guiada</option>
                        <option value="other" <?php echo e(old('type', $event->type) === 'other' ? 'selected' : ''); ?>>Outro</option>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="<?php echo e(old('start_date', $event->start_date->format('Y-m-d'))); ?>" required
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
                <div>
                    <label for="end_date" class="block text-sm font-medium text-gray-300 mb-2">Data de Fim</label>
                    <input type="date" name="end_date" id="end_date" value="<?php echo e(old('end_date', $event->end_date?->format('Y-m-d'))); ?>"
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="<?php echo e(old('start_time', $event->start_time?->format('H:i'))); ?>"
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
                <div>
                    <label for="end_time" class="block text-sm font-medium text-gray-300 mb-2">Hora de Fim</label>
                    <input type="time" name="end_time" id="end_time" value="<?php echo e(old('end_time', $event->end_time?->format('H:i'))); ?>"
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="short_description" id="short_description" value="<?php echo e(old('short_description', $event->short_description)); ?>" maxlength="500"
                       class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" required
                          class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none"><?php echo e(old('description', $event->description)); ?></textarea>
            </div>
            ...
            <div>
                ...
                <?php if($event->image): ?>
                    <div class="mb-4 flex items-center space-x-4">
                        <img src="<?php echo e(asset('storage/' . $event->image)); ?>" alt="" class="h-20 w-32 object-cover rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <label class="flex items-center space-x-3 cursor-pointer">
                        <input type="checkbox" name="is_free" value="1" <?php echo e(old('is_free', $event->is_free) ? 'checked' : ''); ?>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="<?php echo e(old('price', $event->price)); ?>" step="0.01" min="0"
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
                <div>
                    <label for="max_capacity" class="block text-sm font-medium text-gray-300 mb-2">Capacidade Máxima</label>
                    <input type="number" name="max_capacity" id="max_capacity" value="<?php echo e(old('max_capacity', $event->max_capacity)); ?>" min="1"
                           class="w-full rounded-lg bg-admin-bg border border-admin-border px-4 py-2.5 text-white focus:border-accent focus:outline-none">
                </div>
            </div>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laravel_projects\museo\resources\views/admin/events/edit.blade.php ENDPATH**/ ?>