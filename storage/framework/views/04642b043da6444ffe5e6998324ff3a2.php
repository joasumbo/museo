        ...
        <nav class="-mb-px flex space-x-6">
            <a href="<?php echo e(route('admin.visits.index')); ?>" 
               ...
               <?php echo e(!request('status') ? 'border-accent text-accent' : 'border-transparent text-gray-500 hover:text-gray-300'); ?>">
                Todas
                <span class="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-admin-bg"><?php echo e($stats['total']); ?></span>
            </a>
            <a href="<?php echo e(route('admin.visits.index', ['status' => 'pending'])); ?>" 
               ...
               <?php echo e(request('status') === 'pending' ? 'border-accent text-accent' : 'border-transparent text-gray-500 hover:text-gray-300'); ?>">
                ...
                <?php if($stats['pending'] > 0): ?>
                    <span class="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-yellow-500/20 text-yellow-400"><?php echo e($stats['pending']); ?></span>
                <?php endif; ?>
            </a>
            <a href="<?php echo e(route('admin.visits.index', ['status' => 'confirmed'])); ?>" 
               ...
               <?php echo e(request('status') === 'confirmed' ? 'border-accent text-accent' : 'border-transparent text-gray-500 hover:text-gray-300'); ?>">
                Confirmadas
                <span class="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-green-500/20 text-green-400"><?php echo e($stats['confirmed']); ?></span>
            </a>
                        ...
                        <tr class="hover:bg-admin-hover transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    <?php else: ?> bg-gray-500/10 text-gray-400
                                    <?php endif; ?>">
                                    <?php if($visit->visit_type === 'school'): ?> Escolar
                                    <?php elseif($visit->visit_type === 'group'): ?> Grupo
                                    <?php else: ?> Individual
                                    <?php endif; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500"><?php echo e($visit->visit_time->format('H:i')); ?></p>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    <?php else: ?> bg-red-500/10 text-red-400
                                    <?php endif; ?>">
                                    <?php if($visit->status === 'pending'): ?> Pendente
                                    <?php elseif($visit->status === 'confirmed'): ?> Confirmada
                                    <?php elseif($visit->status === 'completed'): ?> Realizada
                                    <?php else: ?> Cancelada
                                    <?php endif; ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <?php if($visit->status === 'pending'): ?>
                                        <form action="<?php echo e(route('admin.visits.confirm', $visit)); ?>" method="POST" class="inline">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Bravantic\Desktop\Museu Municipal Alcanena\resources\views/admin/visits/index.blade.php ENDPATH**/ ?>